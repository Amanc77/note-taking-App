...
const Signup: React.FC = () => {
  ...
  const { signUpInitiator, signUpVerification } = useContext(AuthContext);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    const res = await signUpInitiator(formData);
  ...
  const handleSignUpVerify = async (e: React.FormEvent) => {
    ...
    const res = await signUpVerification({
      otp: inputOtp.otp,
  ...
  return (
    <div className="w-full min-h-screen mx-auto flex flex-col sm:flex-row items-center sm:items-start sm:justify-between gap-4">
      ...
      <div className="w-full sm:w-1/2 max-w-[600px] flex flex-col items-center justify-center gap-1 px-4">
        ...
        <h3 className="font-bold text-2xl mt-3">Sign Up</h3>
        <p className="text-gray-400 text-sm">
          Sign up to enjoy the features of HD
        </p>

        <form
          onSubmit={otpField ? handleSignUpVerify : handleSignUp}
        ...
        >
          ...
          >
            {otpField ? "Verify OTP & Sign Up" : "Get OTP"}
          </button>
        </form>
